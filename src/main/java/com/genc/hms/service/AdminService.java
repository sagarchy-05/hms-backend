...
public class AdminService {
	...
	public List<UserResponseDTO> getAllUsers() {
		return userRepository.findAll().stream().map(user -> new UserResponseDTO(user.getUserId(),
				userService.getRoleIdFromUser(user), user.getEmail(), user.getRole().name())).toList();
	}

	/**
	 * Updates a user's email and/or role. Throws ResourceNotFoundException if user
	 * not found.
	 */
	@Transactional
	public UserResponseDTO updateUserIdentity(Long userId, UserUpdateRoleAndEmailDTO updateDTO) { // 🚨 Direct DTO
																									// return
		...
		return new UserResponseDTO(resultUser.getUserId(), roleId, resultUser.getEmail(), resultUser.getRole().name());
	}

	/**
	 * Resets a user's password. Throws ResourceNotFoundException if user not found.
	 */
	@Transactional
	...
	/**
	 * Deletes a user by ID. Throws ResourceNotFoundException if user not found.
	 */
	@Transactional
	...
	/**
	 * Updates a doctor's profile (non-identity fields, e.g., contact,
	 * specialization). Delegates to the DoctorService method, which should throw
	 * ResourceNotFoundException.
	 */
	@Transactional
	public DoctorResponseDTO updateDoctorProfile(Long doctorId, DoctorProfileUpdateDTO updateDTO) { // 🚨 Direct DTO
																									// return
		// Assuming doctorService.updateDoctorProfile now returns DTO or throws
		// Exception
		return doctorService.updateDoctorProfile(doctorId, updateDTO)
				.orElseThrow(() -> new ResourceNotFoundException("No Doctor Present with Id : " + doctorId));
	}
