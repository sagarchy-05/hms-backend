...
public class BillingService {
     ...
     * Throws IllegalStateException if bill already exists (a business constraint).
     */
    @Transactional
    public BillResponseDTO createInitialBill(Appointment appointment, Doctor doctor) { // 🚨 Direct DTO return
        if (billRepository.findByAppointment(appointment).isPresent()) {
            ...
            throw new IllegalStateException("Bill already exists for appointment ID: " + appointment.getAppointmentId());
        }
     ...
     * Throws ResourceNotFoundException if bill ID is invalid.
     */
    @Transactional
     ...
     */
    @Transactional
    public BillResponseDTO updateBillForDoctorChange(Appointment appointment, Doctor newDoctor) { // 🚨 Direct DTO return
        
        Bill bill = billRepository.findByAppointment(appointment)
                .orElseThrow(() -> new ResourceNotFoundException("Bill not found for appointment: " + appointment.getAppointmentId()));
    ...
    /**
     * Retrieves a specific bill by its ID with patient authorization check.
     * Used by the BillingController's GET /bills/{billId}.
     */
    ...
    /**
     * Retrieves the bill by appointment ID with patient authorization check.
     * Used by the BillingController's GET /bills/appointment/{appointmentId}.
     */
    @Transactional(readOnly = true)
     ...
     * Grants access if user is ADMIN/DOCTOR, or if the user is a PATIENT 
     * and the bill belongs to them.
     */
    ...
    public List<BillResponseDTO> getBillsByPatient(Long patientId) {
        // Note: The controller handles the PATIENT authorization check for this endpoint.
