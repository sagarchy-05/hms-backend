...
public class DoctorService {
	...
	private DoctorAvailabilityDTO mapAvailabilityToDTO(DoctorAvailability availability) {
		return new DoctorAvailabilityDTO(availability.getDayOfWeek(), availability.getStartTime(),
	...
	private DoctorResponseDTO mapDoctorToDTO(Doctor doctor) {
		List<DoctorAvailabilityDTO> availabilities = doctor.getDoctorAvailabilities().stream()
				.map(this::mapAvailabilityToDTO).toList();

		return new DoctorResponseDTO(doctor.getDoctorId(), doctor.getName(), doctor.getSpecialization(),
				doctor.getContactNumber(), doctor.getConsultationFee(), availabilities);
	}
	...
	/**
	 * Updates a doctor's profile and replaces the availability schedule. Clears the
	 * managed collection to ensure JPA orphan removal works correctly.
	 */
	@Transactional
	public Optional<DoctorResponseDTO> updateDoctorProfile(Long doctorId, DoctorProfileUpdateDTO updateDTO) {
		...
		List<DoctorAvailability> newAvailabilities = updateDTO.getDoctorAvailabilities().stream().map(dto -> {
	...
	public List<DoctorResponseDTO> findAllDoctors() {
		return doctorRepository.findAll().stream().map(this::mapDoctorToDTO).toList();
	}
	...
	/**
	 * Searches doctors by name or specialization. Returns all doctors if keyword is
	 * blank.
	 */
	@Transactional(readOnly = true)
	public List<DoctorResponseDTO> searchDoctors(String keyword) {
		...
		return doctorRepository.findByNameContainingIgnoreCaseOrSpecializationContainingIgnoreCase(keyword, keyword)
				.stream().map(this::mapDoctorToDTO).toList();
	}
	...
	public List<WeekDay> getDoctorAvailableDays(Long doctorId) {
		...
		return doctor.getDoctorAvailabilities().stream().map(DoctorAvailability::getDayOfWeek).distinct().toList();
	}
	...
	public List<DoctorAvailabilityDTO> getDoctorAvailability(Long doctorId) {
		return doctorRepository.findById(doctorId)
				.map(d -> d.getDoctorAvailabilities().stream().map(this::mapAvailabilityToDTO).toList()).orElse(null);
	}
