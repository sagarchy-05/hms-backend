...
public class UserService {
	...
	/**
	 * Registers a new Patient and links User -> Patient entities. Cascade ensures
	 * both are persisted in one transaction.
	 */
	@Transactional
	...
	/**
	 * Registers a new Doctor with profile and availability slots. Links User ->
	 * Doctor -> DoctorAvailability entities for persistence.
	 */
	@Transactional
	...
	public UserLoginResponseDTO login(String email, String password) {
		...
		return new UserLoginResponseDTO(user.getUserId(), roleId, user.getEmail(), user.getRole().toString(),
				"Login successful!");
	}
	...
	public boolean changePassword(String authenticatedEmail, UserChangePasswordRequestDTO request) {
		...
		if (!customPasswordEncoder.matches(request.getCurrentPassword(), user.getPassword()))
			return false;
	...
	public List<UserResponseDTO> getAllUsers() {
		return userRepo.findAll().stream().map(user -> new UserResponseDTO(user.getUserId(), getRoleIdFromUser(user),
				user.getEmail(), user.getRole().name())).toList();
	}

	/**
	 * Updates user's email and/or role. Returns Optional.empty() if user not found
	 * or new email conflicts.
	 */
	@Transactional
	public Optional<UserResponseDTO> adminUpdateUserRoleAndEmail(Long userId, UserUpdateRoleAndEmailDTO updateDTO) {
		...
		if (updateDTO.getEmail() != null && !updateDTO.getEmail().equals(user.getEmail())) {
			if (userRepo.findByEmail(updateDTO.getEmail()).isPresent())
				return Optional.empty();
	...
	public UserResponseDTO getUserById(long id) {
		...
		User user = userOptional.get();
		return new UserResponseDTO(user.getUserId(), getRoleIdFromUser(user), user.getEmail(),
